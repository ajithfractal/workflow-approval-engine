...
/**
 * Action to fail a stage - sets completedAt timestamp and updates status to FAILED.
 */
public class FailStageAction implements Action<StageStatus, StageEvent> {

    private final WorkflowStageInstanceRepository workflowStageInstanceRepository;

    public FailStageAction(WorkflowStageInstanceRepository workflowStageInstanceRepository) {
    ...
    public void execute(StateContext<StageStatus, StageEvent> context) {
        ...
        stageInstance.setStatus(StageStatus.FAILED);
